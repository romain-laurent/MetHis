...
void write_vcf_file(param *params, arg *args, unsigned **wanted_samples, unsigned idx_gen){
  ...
  for (i = 0; i < args->nb_snp; i++){
    fprintf(f, "1\t%d\t.\tA\tG\t1\tPASS\tD\tGT", (i+1)*2000);
    nb_done = 0;
    for (j = 0; j < args->sample_size_adm; j++){
      if (wanted_samples[nb_done][0] != FLAG_ADM)
	printf("probleme...\n");
      fprintf(f, "\t%c/%c", params->genos_adm_new[wanted_samples[nb_done][1]][i], params->genos_adm_new[wanted_samples[nb_done][2]][i]);
      nb_done++;
    }
    for (j = 0; j < args->sample_size_s1; j++){
      if (wanted_samples[nb_done][0] != FLAG_S1)
	printf("probleme...\n");
      fprintf(f, "\t%c/%c", params->genos_s1[wanted_samples[nb_done][1]][params->wanted_snps[i]], params->genos_s1[wanted_samples[nb_done][2]][params->wanted_snps[i]]);
      nb_done++;
    }
    for (j = 0; j < args->sample_size_s2; j++){
      if (wanted_samples[nb_done][0] != FLAG_S2)
	printf("probleme...\n");
      fprintf(f, "\t%c/%c", params->genos_s2[wanted_samples[nb_done][1]][params->wanted_snps[i]], params->genos_s2[wanted_samples[nb_done][2]][params->wanted_snps[i]]);
      nb_done++;
    }
    fprintf(f, "\n");
  }
...
void  read_input_genotypes(param *params, arg *args){
  ...
  while (fgets(buff, LARGE_BUFF_SIZE, f_in) != NULL){
    ...
    if (params->genos_s1 == NULL){
      ...
      params->genos_s1 = allocation_char_matrix(params->nb_chrom_s1, params->total_nb_snp);
      params->genos_s2 = allocation_char_matrix(params->nb_chrom_s2, params->total_nb_snp);
    }
    pop_started = 1;
	...
	exit_on_error(NULL, PARSING_ERROR);
      if (wanted_pop == '1')
	params->genos_s1[idx_chrom][idx_snp] = buff[idx_buff];
      else if (wanted_pop == '2')
	params->genos_s2[idx_chrom][idx_snp] = buff[idx_buff];
      idx_buff++;
      idx_snp++;
    }
