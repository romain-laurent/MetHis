...
void set_params_one_simul(param *params, arg *args, unsigned idx_simul){
  ...
  double   mean_mut_rate, mean_param_geom, mean_SNI_rate;
  ...
  /* we draw mutation parameters */
  mean_mut_rate = 5.37e-4;
  mean_param_geom = 2.6e-1;
  mean_SNI_rate = 1.7e-6;
  for (i = 0; i < args->nb_snp; i++){
    params->microsat_mut_rates[i] = gsl_ran_gamma(params->rngs[0], 2, mean_mut_rate/2);
    params->SNI_mut_rates[i] = gsl_ran_gamma(params->rngs[0], 2, mean_SNI_rate/2);
    params->geom_params[i] = gsl_ran_gamma(params->rngs[0], 2, mean_param_geom/2);
  }
}
