...
void make_one_simul(param *params, arg *args){
  ...
  for (idx_gen = 0; idx_gen <= args->nb_generation; idx_gen++){
    ...
    params->genos_adm_new = allocation_int_matrix(2*params->current_adm_Ne, args->nb_snp);
    if (idx_gen != 0){
    ...
    if (idx_gen != 0){
      free_int_matrix(params->genos_adm_old, 2*params->prev_adm_Ne);
      free(params->order_chroms_adm);
    }
    params->genos_adm_old = params->genos_adm_new;
    free_couple_vector(couples, params->current_adm_Ne);
  }
  free_int_matrix(params->genos_adm_old, 2*params->current_adm_Ne);
...
void compute_asd_matrix(arg *args, param *params, unsigned idx_gen){
  ...
  tmp = allocation_char_vector(MEDIUM_BUFF_SIZE);
  sprintf(tmp, "%s/simu_%u/simu_%u_g%u", args->prefix, params->current_simul, params->current_simul, idx_gen);
  sprintf(command, "%s --tfam %s.tfam --tped %s.tped --out %s --threads %u > /dev/null 2> /dev/null", ASD_PATH, tmp, tmp, tmp, args->nb_thread);
  dummy = system(command);
  if (dummy != 0){
...
void *make_reproduction(void *ptr){
  ...
  unsigned idx_couple, idx_snp, real_idx_snp1 = 0, real_idx_snp2 = 0, i;
  couple *tmp_couple;
  unsigned length;
  for (idx_couple = data->idx_min; idx_couple <= data->idx_max; idx_couple++){
    ...
    for (idx_snp = 0; idx_snp < data->nb_snp; idx_snp++){
      ...
      /* then we apply mutation */
      for (i = 0; i < 2; i++){
	/* SNI mutation ? */
	if (gsl_rng_uniform(data->rng) < data->params->SNI_mut_rates[idx_snp]){
	  /* insertion of deletion */
	  if (gsl_rng_uniform(data->rng) < .5)
	    data->params->genos_adm_new[2*idx_couple][idx_snp] += 1;
	  else
	    data->params->genos_adm_new[2*idx_couple][idx_snp] -= 1;
	}

	/* microsat mutation */
	if (gsl_rng_uniform(data->rng) < data->params->microsat_mut_rates[idx_snp]){
	  length = gsl_ran_geometric(data->rng, data->params->geom_params[idx_snp]) - 1;
	  length *= 2;		/* dinucleotides */
	  /* if (length > 20){ */
	  /*   printf("micro %d %e\n", length, data->params->geom_params[idx_snp]); */
	  /* } */
	  /* insertion of deletion */
	  if (gsl_rng_uniform(data->rng) < .5)
	    data->params->genos_adm_new[2*idx_couple][idx_snp] += length;
	  else
	    data->params->genos_adm_new[2*idx_couple][idx_snp] -= length;
	}
